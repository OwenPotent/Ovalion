...
module.exports = {
    ...
    callback: async ({ client, message, args }) => {
        ...
        if (!args[0]) return message.channel.send('That is not a valid userID, please try again!').then(m => m.delete({ timeout: 5000 }));
        ...
        const reason = args[1] ? args.slice(1).join(' ') : 'Unknown Reason';
        ...
        message.guild.fetchBans().then( bans => {
            ...
            if (user) {
                embed.setTitle(`Successfully Unbanned ${user.user.tag}`)
                    ...
                    .addField('Banned Reason', user.reason != null ? user.reason : 'Unknown Reason')
                    .addField('Unbanned Reason', reason)
                message.guild.members.unban(user.user.id, reason).then(() => message.channel.send(embed))
            } else {
                embed.setTitle(`User ${member.tag} isn't banned from the server!`)
                    .setColor('#ff0000')
